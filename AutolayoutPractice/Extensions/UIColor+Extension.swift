//
//  UIColor+Extension.swift
//  AutolayoutPractice
//
//  Created by Alexander Angelov on 4.04.23.
//

import UIKit

extension UIColor {
    static let greyscale140 = UIColor(named: "greyscale140")
    static let greyscale120 = UIColor(named: "greysca;e120")
    static let greyscale100 = UIColor(named: "greyscale100")
    static let greyscale80 = UIColor(named: "greyscale80")
    static let greyscale10 = UIColor(named: "greyscale10")
    static let greyscale05 = UIColor(named: "greyscale05")
    static let brandMainColor = UIColor(named: "brand_color")
    static let brandAccentTitn = UIColor(named: "brandAccentTint")
    static let yellow10 = UIColor(named: "yellow10")
    static let redCritical100 = UIColor(named: "redCritical100")
    static let brandWhite = UIColor(named: "brandWhite")
}
